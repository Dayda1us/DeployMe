...
Write-Output "#                         Version 5.0 (BETA)                          #"
Write-Output "#                      Developed by Charles Thai                      #"
Write-Output "#                                                                     #"
Write-Output "#    FOR TESTING PURPOSES ONLY. DO NOT USE ON PRODUCTION MACHINES.    #"
Write-Output "#######################################################################"
...
# Install a Windows product key for refurbished PCs (1 = Enable, 0 = Disable). You must specify the volume name of the removable media as well as the network drive where it saves the files to.
$installProductKey = 1
...
function Start-DeployMe {
    ...
    END {
        ...
        <#
        # Install a new Windows product key for refurbish PCs
        if ($MARXpress -eq 1) {
            # Prompt the user to point where MARXpress ps1 script is located by its drive letter.
            $Retry = 0
            do {
                $DriveLetter = Read-Host "Please enter the MARXpress drive letter (case insensitive). Please include the double colon (:)"
                if (((Test-Path -Path $DriveLetter\script) -eq $false) -and $Retry -ne 5) {
                    $Retry++
                    Write-Host -ForegroundColor Red "Path does not exist! Please double check which drive letter MARXpress is located."
                } #end if
                elseif ((Test-Path -Path $DriveLetter\script) -eq $true) {
                    Set-Location $DriveLetter\script
                    Invoke-Item .\run.ps1
                }
            } until ($Retry -eq 5)
        
            if ($Retry -eq 5) {
                Write-Warning "You have exceeded the maximum number of retries. Please run MARXpress manually."
            } #end if
        } #end if

        #>

        # Install a new Windows product key and activate the machine.
        if ($installProductKey -eq 1) {
            # Warn and skip this step if the volume name is blank.
            if (($null -eq $USBVolumeName) -or $USBVolumeName -contains "") {
                Read-Host -Prompt "You did not specify the volume name for MARXpress. This process will be skipped." | Out-Null
                exit
            } #end if
