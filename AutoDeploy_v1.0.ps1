...
function Deploy {
    ...
    Write-Verbose -Message "Checks if the script was launched previously."
    ...
    if ($EP -eq "Bypass" -and $NuGet -eq $true -and $PSGallery -eq $true -and $InstallPolicy -eq "Trusted") {
        ...
        Write-Output "Checking if your PC has updates..."
        $GWU = (Get-WUList).Size
        
        Write-Verbose -Message "Switch statement checks if the PC has updates to install by size that is greater than 0 MB."
        switch($GWU) {
            { $GWU -gt 0 } {
                Write-Output "Your PC still has updates to install."
                Start-sleep -Seconds 2
                Clear-Host
                Stage2
                Stage3
            ...
            default {
                Write-Output "Your PC is up to date! Preparing to revert all modified settings back to its original configuration."
                Start-sleep -Seconds 2
                Clear-Host
                Stage3
                Stage4
